...
public class AdminController : ApiControllerBase
{
    ...
    [HttpPost("companies/{id:guid}/suspend")] public async Task<IActionResult> SuspendCompany(Guid id, [FromServices] SuspendCompanyUseCase useCase, CancellationToken ct) => OkData(await useCase.ExecuteAsync(id, CurrentUserId, ct));
    [HttpPost("companies/{id:guid}/reject")] public async Task<IActionResult> RejectCompany(Guid id, [FromServices] RejectCompanyUseCase useCase, CancellationToken ct) => OkData(await useCase.ExecuteAsync(id, CurrentUserId, ct));
    ...
    [HttpPost("job-postings/{id:guid}/reject")] public async Task<IActionResult> RejectJob(Guid id, [FromServices] RejectJobPostingUseCase useCase, CancellationToken ct) => OkData(await useCase.ExecuteAsync(id, CurrentUserId, ct));
}
